...
async def main() -> None:
    ...
    # initiate monthly signals
    @flask_app.route("/initiate-monthly-signals")
    async def initiatemonthly():
        user_email = request.args.get('email')
        full_name = request.args.get('fullname')
        return render_template("initiate_monthly_signals.html", full_name=full_name,user_email=user_email)
    
    
    # initiate 3months signals
    @flask_app.route("/initiate-3months-signals")
    async def initiatequarterly():
        user_email = request.args.get('email')
        full_name = request.args.get('fullname')
        return render_template("initiate_3months_signals.html", full_name=full_name,user_email=user_email)
    
    # initiate 6 months signals
    @flask_app.route("/initiate-6months-signals")
    async def initiatesixmonths():
        user_email = request.args.get('email')
        full_name = request.args.get('fullname')
        return render_template("initiate_6months_signals.html", full_name=full_name,user_email=user_email)
    
    
    # monthly signals
    @flask_app.post("/generate-monthly-signals-link")
    async def generate_monthly_signals_link():
        ...
        if (proceed):
            token=proceed[1]
            return redirect(url_for("monthly_signals",user_id=user_id,token=token))
        else:
             # if the user trying to regenerate another link it returns for them this page
            return render_template("error401.html")
      
    # am  the 1months package 
    @flask_app.route("/monthly_signals/<user_id>/<token>")
    async def monthly_signals(user_id,token):
    ...
    # generate 3months signals
    @flask_app.post("/generate-3months-signals-link")
    async def generate_quarterlysignals_link():
        user_id = request.form['user_id']
        """Checking if current user actually came from Telgram Bot
        False if no 'auth token' replica is available for their user id
        They could have just stored the redirect_url and are getting back
        ...
        if (proceed):
            token=proceed[1]
            return redirect(url_for("threemonthsignals",user_id=user_id,token=token))
        else:
             # if the user trying to regenerate another link it returns for them this page
            return render_template("error401.html")
        
    # am  the 3months package
    @flask_app.route("/3month_signals/<user_id>/<token>")
    async def threemonthsignals(user_id,token):
        chat_id=CHANNEL_CHAT_ID
        proceed = await check_raw_token(token)
        """Checking provided 'auth token' replica is valid
        False if no such 'auth token' exists in system
        ...
        if(proceed):
            ...
            if(subscribed):
                ...
                await add_subscription(user_id,"3month")
                # schedule user removal
    ...
    # generate 6months signals
    @flask_app.post("/generate-6months-signals-link")
    async def generate_halfyearsignals_link():
        user_id = request.form['user_id']
        """Checking if current user actually came from Telgram Bot
        False if no 'auth token' replica is available for their user id
        They could have just stored the redirect_url and are getting back
        ...
        if (proceed):
            token=proceed[1]
            return redirect(url_for("sixmonthsignals",user_id=user_id,token=token))
        else:
             # if the user trying to regenerate another link it returns for them this page
            return render_template("error401.html")
        
    # am  the 6months package
    @flask_app.route("/6month_signals/<user_id>/<token>")
    async def sixmonthsignals(user_id,token):
        chat_id=CHANNEL_CHAT_ID
        proceed = await check_raw_token(token)
        """Checking provided 'auth token' replica is valid
        False if no such 'auth token' exists in system
        ...
        if(proceed):
            ...
            if(subscribed):
                ...
                await add_subscription(user_id,"6month")
                # schedule user removal
